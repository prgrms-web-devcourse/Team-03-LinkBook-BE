...
public class FolderIntegrationTest {
    ...
    void CREATE_FOLDER_TEST() throws Exception {
        CreateFolderRequest createFolderRequest =
                CreateFolderRequest.builder()
                    ...
                    .isPinned(false)
                    .isPrivate(true)
                    .tags(folder1.getFolderTags().stream().map(o -> o.getTag().getName()).collect(Collectors.toList()))
                    .bookmarks(folder1.getBookmarks().stream()
    ...
    void UPDATE_FOLDER_TEST() throws Exception {
        CreateFolderRequest createFolderRequest =
                CreateFolderRequest.builder()
                        .title(folder1.getTitle())
                        .image(folder1.getImage())
                        .content(folder1.getContent())
                        .isPinned(false)
                        .isPrivate(false)
                        .tags(folder1.getFolderTags().stream().map(o -> o.getTag().getName()).collect(Collectors.toList()))
                        .bookmarks(folder1.getBookmarks().stream()
                                ...
                                .collect(Collectors.toList()))
                        .build();
        this.mockMvc.perform(put("/api/folders/{id}", folder1.getId())
                        .characterEncoding(StandardCharsets.UTF_8)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(createFolderRequest)))
                .andExpect(status().isOk())
    ...
    void DELETE_FOLDER_TEST() throws Exception {
        this.mockMvc.perform(delete("/api/folders/{id}",folder2.getId())
                        .characterEncoding(StandardCharsets.UTF_8)
                        .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andDo(
